...
from ..data import _generate_sft_loss_mask, _encode_sample, iter_jsonl, resolve_jsonl_paths
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pack JSONL datasets into MLX binary format")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Output prefix for .ids.bin/.ids.idx/.lbl.bin/.lbl.idx/.meta.json",
    )
    parser.add_argument(
        "--with_labels",
    ...
    ids_bin_path = out_prefix + ".ids.bin"
    ids_idx_path = out_prefix + ".ids.idx"
    ...
    lbl_offset = 0
    t0 = time.time()
    with open(ids_bin_path, "wb") as ids_bin, open(ids_idx_path, "wb") as ids_idx:
        ...
        try:
            for obj in cast(Iterable[Dict[str, Any]], iter_jsonl(paths)):
                ...
                ids_idx.write(
                    (int(ids_offset)).to_bytes(8, byteorder="little", signed=False)
                    + (int(len(ids))).to_bytes(8, byteorder="little", signed=False)
                )
                ids_offset += len(ids)
                ...
                if int(args.log_interval) > 0 and n % int(args.log_interval) == 0:
                    dt = time.time() - t0
                    print(f"[packbin] n={n} {n / max(dt, 1e-6):.0f} lines/s")
        finally:
    ...
    meta = {
        ...
        "ids": {
            "bin": os.path.basename(ids_bin_path),
            "idx": os.path.basename(ids_idx_path),
            "dtype": "int32",
        },
        "labels": None,
    }
